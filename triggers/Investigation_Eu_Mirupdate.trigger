trigger Investigation_Eu_Mirupdate on CMPL123CME__Investigation__c (after insert,after update) {
    ...
    map<string,Id> Inv_Code_Choice_1Map= new map<string,Id>();
     ...
     if(Inv_Code_Choice_1Map.isEmpty()){
        ...
        for(CMPL123CME__Investigation__c cmpinv:[SELECT Id,EU_MIR__c,B_Investigation_Type_Code_Choice_1__c,B_Investigation_Type_Code_Choice_2__c,CMPL123_WF_Status__c   FROM CMPL123CME__Investigation__c WHERE CMPL123_WF_Status__c    ='Closed - Done' AND  B_Investigation_Type_Code_Choice_1__c In:Inv_Code_Choice_1Map.KeySet()]){
